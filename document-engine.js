...
(() => {
  ...
  function num(v,digits=2){return new Intl.NumberFormat('ar-SA',{minimumFractionDigits:0,maximumFractionDigits:digits}).format(Number(v||0));}
  function inventorySummary(s={}){
    ...
    return `<section class="inventory-summary">${cards.map(([k,v],i)=>`<div class="${i===4?'profit-card':''}"><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}</section>`;
  }
  function inventoryTable(items=[]){
    if(!items.length)return '<div class="no-data">لا توجد أصناف مطابقة</div>';
    return `<table class="inventory-table"><thead><tr><th>#</th><th>كود المنتج</th><th>الصنف</th><th>الحالي</th><th>المحجوز</th><th>المتاح</th><th>سعر الشراء</th><th>قيمة الشراء</th><th>سعر البيع</th><th>قيمة البيع</th><th>الموقع</th><th>الحالة</th></tr></thead><tbody>${items.map(x=>`<tr><td>${num(x.index,0)}</td><td class="code">${esc(x.sku)}</td><td class="product-name">${esc(x.name_ar)}</td><td>${num(x.current_qty)}</td><td>${num(x.reserved_qty)}</td><td>${num(x.available_qty)}</td><td>${money(x.purchase_price)}</td><td>${money(x.purchase_value)}</td><td>${money(x.sale_price)}</td><td>${money(x.sale_value)}</td><td>${esc(x.location_code)}</td><td><span class="stock-status ${x.status==='منخفض'?'low':'good'}">${esc(x.status)}</span></td></tr>`).join('')}</tbody><tfoot><tr><th colspan="3">الإجمالي</th><th>${num(items.reduce((s,x)=>s+Number(x.current_qty||0),0))}</th><th>${num(items.reduce((s,x)=>s+Number(x.reserved_qty||0),0))}</th><th>${num(items.reduce((s,x)=>s+Number(x.available_qty||0),0))}</th><th>—</th><th>${money(items.reduce((s,x)=>s+Number(x.purchase_value||0),0))}</th><th>—</th><th>${money(items.reduce((s,x)=>s+Number(x.sale_value||0),0))}</th><th colspan="2"></th></tr></tfoot></table>`;
  }
  ...
  function printCss(size='A4',economy=false,orientation='portrait'){
    ...
    *{box-sizing:border-box}
    body{margin:0;background:#ece9e4;font-family:Tahoma,Arial,sans-serif;color:#33271f;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    ...
    .doc-title{text-align:left;min-width:180px}.doc-title strong{display:block;font-size:21px}.doc-title span{font-family:monospace;font-size:10px;color:#6f6258}
    ...
    th{background:${economy?'#eee':'#e6c477'};color:#2e261f;font-weight:800}th,td{border:1px solid #d9cec4;padding:6px;text-align:right;vertical-align:middle}tr{page-break-inside:avoid}
    ...
    .doc-totals{margin-right:auto;width:min(100%,90mm);display:grid;gap:4px}.doc-totals div{display:flex;justify-content:space-between;border-bottom:1px dashed #ccc;padding:5px}.doc-totals .grand{font-size:14px;background:${economy?'#eee':'#f5e6bf'};border:2px solid #d5aa5b}
    ...
    .qr{width:60px;height:60px;margin-right:auto}.qr:empty{display:none}
    ...
    @media print{body{background:#fff}.no-print{display:none!important}.document{margin:0;box-shadow:none;width:auto;padding:0}.page-counter:after{content:"صفحة " counter(page)}}`;
  }
  function html(doc={},opts={}){
    const h=doc.header||doc.record||{},t=doc.totals||h,title=opts.title||doc.title||labels[doc.type]||labels[opts.type]||'مستند';
    ...
    const table=isInventory
      ...
      :doc.generic?genericTable(doc.items):doc.items?`<table><thead><tr><th>#</th><th>البيان</th><th>الكمية</th><th>سعر الوحدة</th><th>الخصم</th><th>قبل الضريبة</th><th>الضريبة</th><th>الإجمالي</th></tr></thead><tbody>${standardRows(doc.items)}</tbody></table>`:'';
    ...
    return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(title)} ${esc(number)}</title><style>${printCss(opts.size||'A4',opts.economy,orientation)}</style></head><body>
    ...
    <main id="document" class="document ${watermark?'watermarked':''}" data-watermark="${esc(watermark)}">
      ...
      ${h.notes||doc.notes?`<section class="notes"><b>ملاحظات:</b><p>${esc(h.notes||doc.notes)}</p></section>`:''}
      <section class="signatures">${signatures.map(x=>`<div>${esc(x)}</div>`).join('')}</section>
      <footer><span>${isInventory?'تقرير مخزون وردة أشبيليا':'شكرًا لاختياركم وردة أشبيليا'}</span><span class="page-counter"></span><span>${date(new Date())}</span></footer>
    </main>
