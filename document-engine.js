...
(() => {
  ...
  function inventoryTable(items=[]){
    ...
    return `<table class="inventory-table"><thead><tr><th>#</th><th>${tr('كود المنتج')}</th><th>${tr('الصنف')}</th><th>${tr('الحالي')}</th><th>${tr('المحجوز')}</th><th>${tr('المتاح')}</th><th>${tr('سعر الشراء')}</th><th>${tr('قيمة الشراء')}</th><th>${tr('سعر البيع')}</th><th>${tr('قيمة البيع')}</th><th>${tr('الموقع')}</th><th>${tr('الحالة')}</th></tr></thead><tbody>${items.map(x=>`<tr><td>${num(x.index,0)}</td><td class="code">${esc(x.sku)}</td><td class="product-name"><b>${esc(x.name_ar)}</b>${x.name_en?`<small dir="ltr" style="display:block;font-weight:400">${esc(x.name_en)}</small>`:''}</td><td>${num(x.current_qty)}</td><td>${num(x.reserved_qty)}</td><td>${num(x.available_qty)}</td><td>${money(x.purchase_price)}</td><td>${money(x.purchase_value)}</td><td>${money(x.sale_price)}</td><td>${money(x.sale_value)}</td><td>${esc(x.location_code)}</td><td><span class="stock-status ${x.status==='منخفض'?'low':'good'}">${esc(tr(x.status))}</span></td></tr>`).join('')}</tbody><tfoot><tr><th colspan="3">${tr('الإجمالي')}</th><th>${num(items.reduce((s,x)=>s+Number(x.current_qty||0),0))}</th><th>${num(items.reduce((s,x)=>s+Number(x.reserved_qty||0),0))}</th><th>${num(items.reduce((s,x)=>s+Number(x.available_qty||0),0))}</th><th>—</th><th>${money(items.reduce((s,x)=>s+Number(x.purchase_value||0),0))}</th><th>—</th><th>${money(items.reduce((s,x)=>s+Number(x.sale_value||0),0))}</th><th colspan="2"></th></tr></tfoot></table>`;
  }
