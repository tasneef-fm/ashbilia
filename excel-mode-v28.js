...
(() => {
  ...
  pageLabel = function(meta){
    const m={shopbooks:'نظام المحل — عرض الإكسل',pos:'الكاشير',products:'الأصناف والباركود',purchases:'المشتريات والموردون',bookings:'الحجوزات',stocktake:'الجرد الفعلي',damage:'سجل التالف',employees:'الموظفون',attendance:'الحضور',payroll:'الرواتب',settings:'الإعدادات'};
    return m[meta?.id]||oldLabel(meta);
  };
  const oldSub = pageSub;
  pageSub = function(p){
    const m={shopbooks:'نفس ترتيب وطريقة عمل ملفات Excel مع حساب تلقائي',pos:'واجهة بيع مباشرة مرتبطة بالمخزون',products:'دليل الأصناف: الكود، الباركود، الكمية، الشراء والبيع',purchases:'فواتير الموردين والدفعات ومشتريات المخزون',bookings:'كشف الحجوزات: المبلغ، المدفوع والمتبقي',stocktake:'الكمية الفعلية وفرق الجرد واعتماده',damage:'التالف المعتمد وتأثيره على المخزون',employees:'بيانات العامل والراتب الأساسي',attendance:'الحضور والغياب المرتبط بالراتب',payroll:'كشف الرواتب بنفس أعمدة Excel',settings:'الإعدادات الأساسية فقط'};
    return m[p]||oldSub(p);
  };
