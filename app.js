...
function money(v=0){ if(v===null||v===undefined||v==='')return '—'; return new Intl.NumberFormat('ar-SA',{style:'currency',currency:'SAR',maximumFractionDigits:2}).format(Number(v)||0); }
...
async function completeSale(){if(!guard('pos.create_sale'))return;if(!state.posCart.length)return toast('أضف منتجًا واحدًا على الأقل','error');const total=renderPosCart(),customerId=$('#posCustomer').value,customer=state.cache.customers.find(c=>c.id===customerId),discount=Number($('#posDiscount').value)||0,paid=Number($('#posPaid').value)||total;if(discount>0&&!guard('pos.apply_discount'))return;if(paid<total&&!guard('pos.partial_payment'))return;try{const r=await api('/api/orders',{method:'POST',body:{customer_id:customerId||null,customer_name:customer?.name||'عميل نقدي',phone:customer?.phone||'',items:state.posCart.map(i=>({product_id:i.product_id,qty:i.qty,unit_price:i.price})),discount,paid_amount:paid,payment_method:$('#posMethod').value,idempotency_key:crypto.randomUUID()}});toast(`تمت عملية البيع: ${r.item.order_no}`);state.posCart=[];await renderPOS();}catch(err){toast(err.message,'error');}}
...
function workOrderUpdateForm(id){
  ...
  const w=state.cache.workorders.find(x=>x.id===id),emps=state.cache.employees||[];
